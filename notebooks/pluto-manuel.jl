...
# boundary values
phi_left = 0.40      # applied potential at electrode (V) — 
...
sol = nlsolve(residual!, U0; method = :newton, autodiff = :forward, ftol=1e-9, xtol=1e-9, maxiters=200)
...
subplot(4,1,1)
plot(X .* 1e9, phi, "-k", lw=2)
ylabel("ϕ (V)")
title("Potential")

subplot(4,1,2)
plot(X .* 1e9, p ./(1e5), "-b", lw=2)   # in bar-ish
ylabel("p (bar)")
title("Pressure")

subplot(4,1,3)
plot(X .* 1e9, yA, "-b", label="y_A")
plot(X .* 1e9, yC, "-r", label="y_C")
plot(X .* 1e9, y0, "-g", label="y_0")
legend()
ylabel("ey (fraction)")
